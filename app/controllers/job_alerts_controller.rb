class JobAlertsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @job_alert.save
        format.html { redirect_to :back, notice: 'Job Alert Successfully Created.' }
        format.json { render :show, status: :created, location: @job_alert }
      else
  ...
  private
    ...
    def job_alert_params
      params.require(:job_alert).permit(:name, :email, :phone)
    end
end
